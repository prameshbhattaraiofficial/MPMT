...
namespace Mpmt.Services.Services.MailingService
{
    public class MailService : IMailService
    {
        ...
        public async Task<MailServiceModel> EmailSettings(MailRequestModel mailRequest)
        {
            ...
            if (mailRequest.MailFor.ToLower() == "transfer-fund")
            {
                htmlBody += $"Dear {mailRequest.RecipientName}, ";
                htmlBody += $"<p><span style='text-color:purple'>{mailRequest.Content}<span></p>";
                htmlBody += "<br><p>Thank you,<br>Mypay Money Transfer</P>";
            }
            if (mailRequest.MailFor.ToLower() == "password-reset")
            {
                htmlBody += $"";
                htmlBody += $"<p><span style='text-color:purple'>{mailRequest.Content}<span></p>";
                htmlBody += "<br><p>Thank you,<br>MyPay Money Transfer</P>";
            }
            if (mailRequest.MailFor.ToLower() == "eod-balance")
            {
                htmlBody += $"Dear User,";
                htmlBody += $"<p><span style='text-color:purple'>{mailRequest.Content}<span></p>";
                htmlBody += "<br><p>Thank you,<br>MyPay Money Transfer</P>";
            }
            if (mailRequest.MailFor.ToLower() == "exchange-rate-change")
            {
                htmlBody += $"Dear {mailRequest.RecipientName},";
                htmlBody += $"<p><span>{mailRequest.Content}<span></p>";
            }
        ...
        public async Task<bool> SendMail(MailServiceModel mailModel)
        {
            ...
            try
            {
                ...
                smtp.Credentials = new System.Net.NetworkCredential(sMailUserName, sMailPassword); // Enter senders User name and password       
                smtp.EnableSsl = true;
        ...
        public async void SetMailConfiguration()
        {
            ...
            sMailHost = mailConfig.Host;
            iMailPort = mailConfig.Port;
            sMailFrom = mailConfig.MailFrom;
            sMailUserName = mailConfig.UserName;
            sMailPassword = mailConfig.Password;
        }
