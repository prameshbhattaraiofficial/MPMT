...
namespace Mpmt.Services.Common
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string RemoveQueryString(string url, string key, string value = null)
        {
            ...
            var urlHelper = _urlHelperFactory.GetUrlHelper(_actionContextAccessor.ActionContext);
            var isLocalUrl = urlHelper.IsLocalUrl(url);
            var uri = new Uri(isLocalUrl ? $"{url}" : url, UriKind.Absolute);
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContextAccessor.HttpContext.Items[MpmtHttpDefaults.IsPostBeingDoneRequestItem] == null)
                    return false;

                return Convert.ToBoolean(_httpContextAccessor.HttpContext.Items[MpmtHttpDefaults.IsPostBeingDoneRequestItem]);
            }

            set => _httpContextAccessor.HttpContext.Items[MpmtHttpDefaults.IsPostBeingDoneRequestItem] = value;
        }
        ...
        {
            var baseurl = _httpContextAccessor.HttpContext.Request.Host.Value.ToString();
