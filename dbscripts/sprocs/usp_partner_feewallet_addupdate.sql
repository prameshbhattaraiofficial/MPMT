USE [MpmtDb]
GO
/****** Object:  StoredProcedure [dbo].[usp_partner_wallet_addupdate]    Script Date: 9/12/2023 10:27:17 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
Create   PROCEDURE [dbo].[usp_partner_feewallet_addupdate]     
(    
  @Id int = 0     
...
BEGIN
	...
	BEGIN	
		...
		IF EXISTS(SELECT 1 FROM dbo.tbl_partner_fee_account WITH(NOLOCK) WHERE PartnerCode=@PartnerCode AND SourceCurrency=@SourceCurrency)
		BEGIN
		...
		INSERT INTO [dbo].tbl_partner_fee_account
		(
			PartnerCode,SourceCurrency,Balance,NotificationBalanceLimit,MarkupMinValue,MarkupMaxValue,TypeCode
			,Remarks,IsActive,IsDeleted,UserType,CreatedById,CreatedByName,CreatedDate
		)
		OUTPUT inserted.Id INTO @TblWallet
		VALUES
		(
			@PartnerCode,@SourceCurrency,0,@NotificationBalance,@MarkupMinValue,@MarkupMaxValue,@TypeCode
	...
	BEGIN    
		...
		BEGIN    
			...
			RETURN;    
		END 
		IF EXISTS(SELECT 1 FROM dbo.tbl_partner_fee_account WITH(NOLOCK) WHERE PartnerCode=@PartnerCode AND SourceCurrency=@SourceCurrency  AND Id<>@Id)
		BEGIN
			SET @MsgText = 'Specified Wallet is already present!'    
			RETURN;  
		END
		...
		UPDATE [dbo].tbl_partner_fee_account
		SET
